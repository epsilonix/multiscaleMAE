...
def plot_umap_mosaic(umap_path, plot_path, dataloader = None, vis_codex = None):
    img_size = 128
    ...
    for idx_x in tqdm(range(frame_x // bin_size)):
        for idx_y in range(frame_y // bin_size):
            ...
            if len(qualified_patches) > 0:
                ...
                img_array = np.array(torch.moveaxis(img_tensor, 0, 2).detach().cpu())
                img_array = vis_codex(img_array * 8)
                img_array = img_array.astype(np.uint8)[:,:,:3]
